...
export default function PartnerPostCard({ post }) {
  ...
  const isPriority = post.postType === 'PRIORITY';

  return (
    <Link 
      to={`/partner-posts/${post.id}`} 
      className={`group block bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border h-full flex flex-col ${isPriority ? 'border-yellow-200 ring-1 ring-yellow-100' : 'border-gray-100'}`}
    >
      {/* Image */}
      <div className="relative h-56 overflow-hidden">
        ...
        {isPriority && (
            <div className="absolute top-3 left-3 z-10">
                <span className="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide shadow-md bg-gradient-to-r from-yellow-400 to-orange-500 text-white flex items-center gap-1">
